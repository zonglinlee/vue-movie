    ...
    <template v-if="lists">
      <header v-if="lists">
        ...
        <a target="_blank" href="https://github.com/wclimb/vue-video">
          <svg class="github" fill="#FFF" height="32" version="1.1" viewBox="0 0 16 16" width="32">
            <path
              fill-rule="evenodd"
      ...
      <section class="video_txt">
        <div class="video_txt_wrap">
          <p>
            <template v-if="lists.timelong > 90">{{ lists.timelong }}分钟</template>
            <template v-else>{{ lists.timelong }}季</template>
            {{ lists.type }}
          </p>
      ...
      <section class="video_comments">
        ...
        <ul id="ul">
          <li :key="comment.id" v-for="comment in comments">
            ...
            <template v-else>
              <div class="avator">{{ comment.userName.charAt(0) }}</div>
            </template>
